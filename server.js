...
// ── Proxy de mídia com cache inteligente ──
// Para imagens e áudios: cache em memória (rápido)
// Para vídeos: baixa o arquivo inteiro e cacheia (evita buffering infinito)
const mediaUrlCache = new Map(); // URL cache (10 min)
const _mediaBufCache = {};       // buffer cache (imagens, áudios e vídeos)
...
app.get("/media-proxy/:mediaId", async (req, res) => {
  ...
  try {
    // 1. Obter URL da mídia (cache de 10 min)
    ...
    // 2. Determinar se é vídeo (para cache especial)
    const isVideo = /\.(mp4|mov|avi|mkv|webm)$/i.test(filename || '');
    const isImage = /\.(jpg|jpeg|png|gif|webp)$/i.test(filename || '');
    const isAudio = /\.(mp3|ogg|wav)$/i.test(filename || '');

    // 3. Se for vídeo OU imagem/áudio, usamos cache em memória (arquivo completo)
    // Isso evita o buffering infinito e é mais rápido para vídeos pequenos (até 16MB)
    let entry = _mediaBufCache[cacheKey];
    const now = Date.now();
    if (!entry || now - entry.ts > 600000) { // 10 minutos
      const resp = await axios.get(url, {
        headers: { Authorization: `Bearer ${token}`, "User-Agent": "WhatsApp/2.0" },
        responseType: "arraybuffer",
        timeout: 120000, // 2 minutos para baixar vídeos grandes
      ...
      // Limita o cache a 5 arquivos (prioriza os mais recentes)
    ...
    // 4. Servir o buffer (com suporte a Range)
    const buf = entry.buf;
